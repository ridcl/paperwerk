...
LAYOUT
1. Mirror the visual layout of the input: headings, columns, dividers, font weights, alignment, relative spacing, section ordering. Use only system fonts: "DejaVu Serif", "Liberation Serif", "DejaVu Sans", "Liberation Sans", Georgia, Helvetica, Arial. DO NOT @import Google Fonts or load any external resource. Where the source shows a company logo or brand mark, do NOT reproduce it. Never copy the source's logo — not its wordmark, not its icon, and not its shape or color scheme (for example, if the source logo is four colored squares, you must NOT draw four colored squares). Instead INVENT an original, plausible-looking logo for a different, fictitious company and place it at roughly the same size and position. Make it look like a real brand mark, not a placeholder: e.g. a tasteful monogram of the issuer's initials inside a rounded-rect or circular badge, or a simple abstract emblem, in one or two coherent brand colors of YOUR choosing (clearly different from the source's colors). Prefer deriving the monogram letters from the issuer via a Jinja expression (e.g. an SVG <text> node containing {{ issuer_name[:1]|upper }}) so it matches the generated company. Build it as a small inline SVG.
