...
Task:
  1) Pick up to 10 of the MOST USEFUL facts - the figures a real user would \
actually search for. Skip near-duplicates; when the same concept appears for \
several periods or segments, you may include more than one ONLY if the period \
or segment clearly distinguishes them.

  2) For each chosen fact, write ONE query: what a real user would type to find \
that figure. Use the concept, and the period or segment when needed to make the \
query unambiguous (e.g. "total net sales for fiscal 2025", "cash and cash \
equivalents at year end"). Phrase it as a natural question or a short search \
phrase. Do NOT include the raw value in the query.
     - The reader already knows which company's filing this is, so do NOT name \
the filing entity in the query: write "total net sales for fiscal 2025", NOT \
"Apple's total net sales for fiscal 2025". Only include a company/entity name \
when the figure belongs to a DIFFERENT named entity mentioned on the page (e.g. \
a subsidiary, an equity-method investee, or an acquired company) and the name is \
needed to say which one the figure is about.

  3) A single query MAY point to multiple facts when they are equally valid \
evidence for the same question (e.g. a value that appears in two places). List \
their ids together under one query. Otherwise one id per query.
...
def _resolve_filings(
    tickers: list[str], forms: tuple[str, ...], per_company: int, limit: int
) -> list[Filing]:
...
async def _run(
    ...
    print(f"resolving filings for {len(tickers)} ticker(s), forms={forms} ...")
    filings = _resolve_filings(tickers, forms, per_company, limit)
    if not filings:
        sys.exit("error: no filings resolved")
    print(f"{len(filings)} filing(s) to process")

    # Phase 2-3: download + render + locate + window (bounded concurrency).
    render_sem = asyncio.Semaphore(_RENDER_CONCURRENCY)
    rendered = await asyncio.gather(
        *(_render_filing(f, render_sem) for f in filings)
    )

    # Flatten to one LLM task per (filing, window).
    ...
    if not tasks:
        sys.exit("error: no windows planned (no dense pages found)")
    print(f"{len(tasks)} window(s) -> querying LLM ...")
    ...
    try:
        for task, out in zip(tasks, llm_outputs):
            ...
            assembled = _assemble_datapoint(
                out, facts_in_window, task.win_start, rng.shuffle
            )
            ...
            if any(p not in page_png for p in range(task.win_start, task.win_end + 1)):
                continue
            images = [
                page_png[p] for p in range(task.win_start, task.win_end + 1)
            ]
    ...
    print(f"\nwrote {output_path}: {n_written} datapoint(s) from {len(filings)} filing(s)")
...
def main() -> None:
    ...
    if not tickers:
        sys.exit("error: --tickers must list at least one ticker")
    asyncio.run(
        _run(
            tickers=tickers,
            forms=forms,
