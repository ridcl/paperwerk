...
# Training data is the concatenation of two HF Hub datasets (same VQA schema),
# loaded in load_splits(). No KVP10k, and no local parquet.
CUAD_DATASET = "ridcl/cuad-synthetic-vqa"
XBRL_DATASET = "ridcl/xbrl-tables"
...
def load_splits() -> tuple[pd.DataFrame, pd.DataFrame]:
    # Concatenate the two source datasets; they share the VQA schema exactly
    # (CUAD-synthetic is single-page, XBRL-tables is multi-page windows), so the
    # downstream windowing/split logic treats them uniformly. `source` strings
    # don't collide across the two (CUAD: "<class>_<variant>_<idx>"; XBRL:
    # "sec_<...>_<window>"), so the document-level split below stays well-defined.
    df = pd.concat(
        [_hf_to_df(CUAD_DATASET), _hf_to_df(XBRL_DATASET)], ignore_index=True
    )
